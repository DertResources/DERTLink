...
public:
        ...
        inline DeviceBuilder& BuildNewDevice(std::string deviceName)
        {
            deviceManifests.emplace_back(deviceName, this);
            return deviceManifests.back();
        }

        inline bool ValidateManifest()
        {
                ...
                [&](DeviceBuilder& db) -> bool {
                    ...
                    std::for_each(db.GetEntryManifests().begin(), db.GetEntryManifests().end(),
                        [&](EntryManifest& em) {
                            ...
                            if (deMatchIt == devDict.second.end() ||
                                std::visit([&](auto& de) { return de.GetName(); }, **deMatchIt) != em.entryName)
                            {
                                valid = false;
                                return;
                            }
                                ...
                                if (em.entryData.has_value())
                                {
                                    ...
                                    std::visit([&](auto& data) {
                                        ...
                                        auto dt = de.GetDataType();
                                             if (dt == DataType::UINT8 ) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<uint8_t>(data); }
                                        else if (dt == DataType::UINT16) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<uint16_t>(data); }
                                        else if (dt == DataType::UINT32) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<uint32_t>(data); }
                                        else if (dt == DataType::UINT64) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<uint64_t>(data); }
                                        else if (dt == DataType::INT8  ) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<int8_t>(data); }
                                        else if (dt == DataType::INT16 ) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<int16_t>(data); }
                                        else if (dt == DataType::INT32 ) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<int32_t>(data); }
                                        else if (dt == DataType::INT64 ) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<int64_t>(data); }
                                        else if (dt == DataType::FLOAT ) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<float>(data); }
                                        else if (dt == DataType::DOUBLE) { if constexpr (std::is_arithmetic_v<T> && !std::is_same_v<T, bool>) em.entryData = static_cast<double>(data); }
                                        else if (dt == DataType::BOOL  ) { if constexpr (std::is_same_v<T, bool>)        em.entryData = data; }
                                        else if (dt == DataType::STRING) { if constexpr (std::is_same_v<T, std::string>) em.entryData = data; }
                                        }, em.entryData.value());

                                } else
                                {
                                    bool dataDirectionMatched = (de.GetDataDirection() == DataDirection::DESIREDSTATE ||
